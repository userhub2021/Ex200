...
本リポジトリに同梱されている [ExSetup.go](file:///root/Ex200/Ex200_v2/ExSetup.go) (環境構築プログラム) および [verify_rhcsav2.go](file:///root/Ex200/Ex200_v2/verify_rhcsav2.go) (採点プログラム) は、Go 言語 (Golang) で記述されています。
...
1. **プログラムのビルド:**
   ...
   ```
   * このコマンドを実行すると、同じディレクトリに `ExSetup` および `verify_rhcsav2` という名前の実行可能ファイルが生成されます。
...
* 生成された `ExSetup_static` と `verify_rhcsav2_static` は他の RHEL 10 サーバーへ転送して、即座に実行できます。
